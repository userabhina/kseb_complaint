...
public class ViewReport extends HttpServlet {
	...
	public void service(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			pst = conn
					.prepareStatement("SELECT consumer_id FROM consumer_personal WHERE consumer_name='" + uname + "'");
			rs = pst.executeQuery("SELECT consumer_id FROM consumer_personal WHERE consumer_name='" + uname + "'");
			rs.next();
			int cons_id = rs.getInt(1);

			pst = conn.prepareStatement(
					"SELECT comp_date,complaint,comp_status FROM complaint WHERE consumer_id=" + cons_id + "");
			rs = pst.executeQuery(
					"SELECT comp_date,complaint,comp_status FROM complaint WHERE consumer_id=" + cons_id + "");
			...
			out.println("<div class=\"complaintTable\">\r\n" + "<table class=\"comp\">" + "<tr><th>Complaint date</th>"
					+ "<th>Complaint</th>" + "<th>Status</th>" + "</tr>");
			while (rs.next()) {
