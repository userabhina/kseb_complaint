...
public class ManagerPanel extends HttpServlet{
	...
	public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			pst = conn.prepareStatement("SELECT * FROM material WHERE material_eng_accepted=1");
			rs=pst.executeQuery("SELECT * FROM material WHERE material_eng_accepted=1");
			out.println("<html>");
			...
			while(rs.next()) {
				  ...
				  out.println("<form action=\"managerAcceptance\" method=\"post\">");
				  out.println("<h3>Request ID:</h3> <div class=\"rbox\"><h2>"+rs.getInt(1)+"</h2></div>");
				  out.println("<input type=\"hidden\" id=\"rID\" name=\"reqID\" value="+rs.getInt(1)+">");
